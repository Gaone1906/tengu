 ...
 *  Attention board; the people lens only exists on the legacy list; everything
 *  else lands on My requests. Remaining search params ride along so filtered
 *  bookmarks stay filtered. Pure — unit-tested directly. */
export function legacyTodosRedirectTarget(search: string): string {
  ...
  if (view === "people") return `/todos/list?view=people${rest ? `&${rest}` : ""}`
