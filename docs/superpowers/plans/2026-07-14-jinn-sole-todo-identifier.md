# JIN-N Sole Todo Identifier Implementation Plan — v5 Conditional Architecture
...
| Select the browser-visible canonical-ID policy | **A** | **A (recommended):** every syntactically exact `JIN-N` token in proven machine transcript prose renders as non-linking text, and structured Todo identity/copy/search/API/MCP output may show it. **B:** suppress `JIN-N` from all DOM/rendered text and use title-only UI. Both forbid canonical IDs in browser navigation/history/storage/hidden attributes/technical anchors/cursors/unrelated metadata and neutralize historical machine legacy literals |

The common privacy floor is locked; only the mechanically specified A/B rendering branch is undecided. Task 0 cannot close until the operator records an answer for all ten rows, including concrete `G` and `M` semvers selected by the two-stage row and an explicit `A` or `B`, and an independent reviewer approves this revision. Until then this plan is **CONDITIONAL** and architecture GO is false.

### Conditional browser policy A/B

**A — mechanically visible canonical tokens (recommended).** Every boundary-delimited token that satisfies exact `^JIN-[1-9][0-9]*$` in **proven machine transcript prose** renders as visible, non-linking text. The presenter does not infer intent, consult Todo existence, validate instance ownership, inspect receipts, or distinguish real, hallucinated, nonexistent, wrong-instance-looking, and coincidental tokens. Structured Todo rows/detail/Activity/delegation cards, their matching accessible names, explicit Copy ID/textual share output, Todo identity search/results, and explicit authenticated API/MCP identity fields may also show `JIN-N`. Canonical queries remain ephemeral and never enter browser location/history/storage. Technical metadata, tool correlation, anchors, attributes, URLs, and cursors remain private or opaque even when their raw value happens to match the grammar.
...
Both policies preserve user-authored source bytes byte-for-byte and treat authored literals as non-resolving text. Policy A renders proven authored prose byte-identically; Policy B keeps source/API evidence intact but masks canonical tokens in browser presentation. Historical machine-authored exact legacy `wi_*` literals are deterministically replaced with the same neutral `[historical Todo reference]` marker under **both** policies. They are never mapped to `JIN-N`, even during cutover, so presentation rebuild never needs the temporary migration map or audit manifest. Unknown and formerly mapped legacy tokens have identical output. Current/post-cutover exact `JIN-N` differs only by A/B. The two policies differ downstream as follows:

| Surface | Policy A | Policy B |
|---|---|---|
| Transcript | Historical machine legacy tokens neutralize; every exact current `JIN-N` token in proven machine prose renders as non-linking text without semantic/existence lookup | Historical machine legacy and every rendered current `JIN-N` token neutralize |
...
| Ordinary `messages.content`, including user/assistant/notification prose, tool input/result, partial/fallback text | Turns, callbacks, blocks, connectors, native transcript sync | DB reads, FTS, REST/MCP list/search/context/read-session, WebSocket, connectors, engine switch/re-prompt | Immutable authored/evidence bytes plus derived live presentation | Preserve raw bytes. Proven human text is non-resolving and renders byte-identically under A; B masks exact canonical tokens in browser display. Historical machine legacy tokens always neutralize map-free; current exact canonical tokens pass under A and mask under B. `role` never proves authorship |
| `messages.media` JSON (`type`, `url`, `name`, `mimeType`, `size`) | Attachment upload/message insertion, duplication, native/import paths | `rowToMessage`, REST/MCP/session reads, web attachment labels/links, connector/engine ingress | Mixed authored label plus technical structure | Preserve raw JSON. Full-scan raw/decoded keys/values. Per element, provenance marks `name` independently; A preserves proven authored label and exposes exact canonical prose tokens, B masks them. Machine legacy labels neutralize both. URL/MIME/type/size are technical: exact Todo tokens are rejected or converted to existing attachment capabilities, never substring-rewritten into a broken URL |
| `messages.tool_call` | Streaming engine partial/tool producer, duplication/backfill | Reloaded tool-card name, REST/MCP/WS, engine context | Machine protocol name, never transcript prose | Preserve raw internally for the active adapter only. Public/restored projection derives a non-executable display label through a closed safe-name validator; any exact legacy or canonical Todo-shaped token becomes neutral `Tool` under **both** A and B. Stored history never replays the display label as an executable tool call |
| `messages.tool_id` | Streaming engine tool-call/result correlation, duplication/backfill | Reload grouping, settlement, REST/MCP/WS, engine correlation | Machine technical correlation | Never expose or text-sanitize raw ID. Project to a stable current-epoch opaque `tc_*` correlation using the instance projection key plus session/raw ID, and use the same projection for call/result pairs. Same active adapter keeps raw ID privately; restored engine input uses only safe projected correlation or a neutral non-executable summary |
| `messages.content_origin`, `media_label_origins` (one entry per media element), `tool_call_origin`, and `tool_id_origin` | Guard-release message producer boundary | Presentation, FTS, public serialization, engine prompt, migration preflight | Authoritative per-field provenance | Closed enum `human-authored | machine-generated | external-native | unknown` is stamped atomically for every populated source column; role is never provenance. Human upload labels, agent/MCP labels, engine tool fields, external/native fields, and duplicates use explicit producer values. `unknown` is permitted only for proven identity-free historical data; identity-bearing ambiguity blocks. Raw columns remain frozen |
...
| `message_presentations(message_id, identity_epoch, sanitizer_version, api_json, browser_json, engine_json)` | Offline cutover and every current-epoch message insert/update | Safe FTS, REST/MCP/web/connector output, engine switch/re-prompt | Deterministically derived/rebuildable complete-message projection | Each JSON contains authoritative projected `content`, `media`, `toolCall`, opaque `toolRef`, `blocks`, and `meta` for that boundary. The pure rebuild takes only frozen row bytes, durable per-field provenance, current identity epoch/policy/sanitizer version, and persistent non-Todo projection key—never the temporary Todo map or audit manifest. No reader falls back to raw columns |
| `messages_fts`, triggers, backfill watermark, snippets | Registry insert/update and lazy historical backfill | REST/MCP search and context entry | Derived/rebuildable index | Drop raw-content index/triggers/watermarks; rebuild before listen from `message_presentations.api_json.content`; stamp epoch; mismatch/corruption refuses and no post-listen raw-source backfill runs. Browser result rendering passes snippets through the browser presentation policy |
| `sessions.engine_session_id`, `sessions.engine_sessions[*].id`, and engine sync metadata | Engine hooks, engine switching, native transcript adoption/backfill/tail sync | Resume, retry, switch, copyable resume command, transcript reader | Capability pointers to a binding, not read authority | Stamp with identity epoch and binding generation. A naked native ID/path never authorizes transcript reads. Sever pre-cutover refs and bindings; old refs never serialize, resume, backfill, tail-sync, or transfer to duplicates |
| `native_transcript_bindings` (`session/engine/generation`, epoch, authenticated native session/attempt proof, canonical allowed path, file identity, adoption size/start offset/expected size, prefix digest, committed complete-line offset, status) | Current authenticated engine turn/hook adoption | Raw-safe transcript API, backfill/tail sync, late recovery, native resume | Authoritative current-epoch file capability | Bind only an opened regular file under the engine-specific root. Device/inode or platform file ID plus SHA-256 prefix proves identity. Offset/digest advance atomically with projected DB writes. Rotation/replacement/truncation freezes the binding; a new file requires fresh authenticated adoption |
| Native Claude/Codex/Grok/Antigravity JSONL/transcript bytes | External engines and transcript tailers | Safe transcript route, backfill/tail sync, late recovery, native resume | External/frozen evidence | Never rewrite. Pre-cutover files/bindings become inert. Current readers use only a verified binding/descriptor, never path rediscovery, and normalize each accepted append before output, DB, FTS, connector, or prompt use |
...
| Legacy Workflow `state.json`, `reports/waves/wave-N.json`, and `workflows/<id>.workflow.yaml` | Legacy derive/report paths | Legacy presentation/derivation and compatibility | Immutable or executable historical artifacts | Include in the closed artifact allowlist. JSON receives raw-plus-decoded traversal; YAML receives raw scan plus safe parse/recursive decoded traversal. Any executable/nonterminal exact legacy token blocks; terminal evidence stays byte-identical behind presentation |

Task 4 requires legacy synthetic Workflow Session rows, messages, queue entries, and callbacks to remain byte-identical. Task 5 gives accepted callback deliveries immutable evidence semantics. Therefore a blanket historical rewrite is forbidden. The smallest safe content rule is unconditional: `scanWorkflowArtifact` scans raw bytes with shared `findLegacyTodoIds`, parses the expected JSON/YAML format once, then walks the complete decoded tree and applies the same finder/revalidator to every string value and own-property key. Invalid syntax, unsupported depth/size, traversal truncation, or inability to prove status fails closed; it never silently skips or recursively reparses strings.

The scanner also owns a hostile-filesystem boundary. Its closed allowlist is exactly: `<root>/workflows/<full-safe-id>.definition.json`, legacy `<root>/workflows/<full-safe-id>.workflow.yaml`, `<root>/workflow-triggers/triggers.json`, `<root>/reports/runs/_active-index.json`, `<root>/reports/runs/<full-safe-id>/<full-safe-id>.json`, `<root>/reports/run-idempotency/<64-lowercase-hex>.json`, legacy `<root>/state.json`, and `<root>/reports/waves/wave-(0|[1-9][0-9]*).json`. Full safe IDs use the existing 1–128 character checks plus `^[A-Za-z0-9][A-Za-z0-9._-]*$`; unsafe or unexpected managed-suffix names, temp files, and ambiguous artifact classes fail closed. Optional absent stores are allowed. It never recursively reads arbitrary `reports/**`.

Before enumerating, lstat the configured root and every expected parent, require stable real directories, and reject symlinks. For each candidate: lstat; reject symlink/non-regular; open read-only with platform `O_NOFOLLOW|O_NONBLOCK` and no fallback; immediately fstat; require the same `{dev,ino,mode,size,nlink,mtimeNs,ctimeNs}` identity and revalidate the parent chain. Read fixed chunks from the opened descriptor only, under versioned per-file plus aggregate file/byte budgets; reject pre-size above cap, sparse/oversize, early EOF, one byte beyond expected size, or post-read fstat/lstat/parent identity/type/size/time drift. Never reopen by path. Unsupported no-follow/stable-identity semantics is migration no-go. Diagnostics contain only artifact class, validated store-relative locator, phase/reason code, safe artifact digest when available, and hashed structural locator/match; never absolute/foreign paths, symlink targets, raw tokens/keys/values, or decoded prose.

Preflight refuses migration when any nonterminal `running`, `parked`, or `dispatched` run contains any shared-parser exact legacy token—mapped or unknown—in either representation. This includes quiet parked runs, parked runs with in-flight siblings, stopping drains, trigger payload object keys/values, normalized `triggerTodoId`, frozen condition paths/values, prompts, parameters, overrides/edits, receipt outcomes/field keys/values, parked/errors/report text, and runs omitted from or wrongly present in the active index. Mutable definitions, custom-trigger stores, invocation claims, legacy state/waves/YAML, and every other allowlisted Workflow artifact receive the same safe scan: typed live Todo fields are rekeyed only when separately classified, user strings remain bytes, and any unclassified decoded/raw exact match blocks. `_active-index.json` is derived and rebuilt only from clean parsed runs, never trusted as the scan source.
...
Guard release adds mandatory per-field origin at every message producer; `role='user'` is not proof. The producer manifest marks authenticated human/connector ingress as human only when its call path carries explicit authorship, and marks Workflow spawn/follow-up, delegation/agent-created prompts, callback/queue replay, platform context, block fallback, notifications, assistant/tool output, and native backfill/tail/recovery as machine or external-native. Media labels are classified independently from their technical descriptor; tool names/IDs are machine protocol fields. Historical classification uses persisted session kind/source, callback/queue/block correlation, native sync anchors, and producer-specific metadata only when the exact shape proves each field. An identity-token-bearing ambiguous field blocks; it is never silently labeled human. An unknown non-bearing field may use conservative machine presentation while raw columns remain frozen.

Presentation rebuild is deliberately map-free. `buildMessagePresentation(rawRow, fieldOrigins, identityEpoch, policy, sanitizerVersion, projectionKey)` is a pure deterministic function. Historical machine legacy tokens always become `[historical Todo reference]`; it never accepts an old→new map and cannot read the audit manifest. The persistent projection key is scoped only to opaque `tc_*` tool correlation and cannot resolve a Todo. Cutover drops the temporary map, makes the manifest unavailable to runtime, deletes/rebuilds the entire projection, and byte-compares output. A sanitizer-version bump rebuilds twice from the same frozen rows and provenance and must produce identical rows for that version. Missing/corrupt presentation is therefore repaired offline without recovering old identity mapping.

`media` is strictly decoded against the closed `MessageMedia` keys/types and its origin-array cardinality; invalid JSON, unknown keys/types, unsafe URL, limit overflow, or collision fails closed rather than disappearing. Proven human attachment `name` follows A/B authored-label rules; agent/engine labels follow machine rules. `type`, URL, MIME, and size are technical and may serialize only from validated same-instance attachment capabilities—never a Todo-bearing raw URL. FTS remains content-only; a token solely in media/tool fields is not indexed, and search→context/page still returns only the complete safe projection.

Raw `tool_id` is internal engine correlation and is never text-neutralized or serialized. Derive `tc<identityEpoch>_<HMAC-SHA256(projectionKey, sessionId || NUL || rawToolId)>` with collision checks; the same call/result pair shares it, different raw IDs or duplicated sessions do not. Public REST/MCP/WS/web uses `toolRef`; the active adapter may retain raw ID privately. Restored history/engine context uses projected correlation or neutral non-executable summaries and never replays raw stored tool protocol. Raw `tool_call` is also technical protocol metadata, not Policy-A prose: a closed safe-name validator may derive a display-only `toolCall` label, but any exact legacy or current Todo-shaped token becomes neutral `Tool` under both policies, and the projected label is never executable. Policy A's grammar-pass rule applies to proven machine `messages.content` prose and separately proven authored attachment labels only. Duplication explicitly copies media/tool fields plus provenance, then derives new session-scoped tool refs; until guard release, the current omission of copied tool fields is a tested legacy fact rather than assumed coverage.
...
Replace raw-content `messages_fts` with an epoch-scoped FTS over the validated `content` member of `message_presentations.api_json`. Cutover drops old triggers/watermarks, fully rebuilds before listen, and validates its epoch and zero machine-presentation legacy identities; explicitly authored user literals may remain non-resolving API/MCP search text. Snippets come only from that projection, and the web applies the browser presentation policy before rendering them. No lazy/post-listen legacy backfill may re-index raw source directly; corrupt/missing/mismatched FTS epoch refuses search and serving until an offline safe rebuild completes.

Native histories are external frozen bytes and capability-bearing ingress. Add `sessions/native-transcript-bindings.ts`; stamp scalar `engine_session_id`, every `engine_sessions[engine]` entry, queue/retry context, and tailer state with the Todo identity epoch plus binding generation. At cutover, inventory but sever every epoch-1 binding/ref and clear its sync/resume authority; do not serialize old refs or the copyable native `--resume` command. `/api/sessions/:id/transcript` becomes a safe current-binding presentation or returns `410`; no raw JSONL endpoint remains. Empty-session backfill, on-load tail sync, hook adoption, late recovery, engine switching, retry, pending replay, and normal resume require a current verified binding. First post-cutover use of an old session starts fresh.

Fresh adoption is permitted only from a current-epoch authenticated engine turn/hook bound to the Jinn session and attempt. A hook path or `findTranscriptForSession` result is only a candidate during adoption. Resolve it under the engine-specific allowed root/name contract; lstat rejects symlink/non-regular; open `O_RDONLY|O_NOFOLLOW`; fstat and match path identity. Persist canonical path, identity kind, device/inode where meaningful (or a platform-stable file ID), adoption size `S`, start/committed offset `O`, last expected size, SHA-256 of bytes `[0,O)`, epoch, native session/attempt proof, and generation. Default tail adoption sets `O=S`, so pre-adoption bytes never enter; explicit authenticated safe-backfill adoption may choose `O=0`. If stable identity is unavailable, adoption may use the open descriptor but descriptor loss requires fresh adoption—path alone never permits reopen.

Every read lstat-checks the path and fstat-checks the held descriptor, requires regular type and the bound identity, verifies safe integer and configured total/append bounds, rejects size below `O` or prior expected size, recomputes SHA-256 over `[0,O)`, then reads only a snapshotted `[O,size)` and fstats again. Only complete JSONL lines commit; projected message writes plus new complete-line offset/digest/expected size share one SQLite transaction. Watch/stat events are wake hints, never authority. Disappearance, repoint, rotation, identity drift, truncation/regrow, prefix rewrite, short/inconsistent read, nonregular type, or impossible growth freezes the binding and emits/inserts nothing. After read error, reopen no-follow only when path, stable file identity, regular type, size, and committed digest still match; otherwise fresh authenticated adoption is mandatory. The same native session ID pointing to a new file never auto-adopts.

Engine switching and re-prompt use `engine_content`, never raw `getMessages`; pending prompts carry the epoch. Migration requires no active/waiting/partial/pending old-epoch turn. A stateful machine-content normalizer buffers across streamed delta boundaries so split exact tokens cannot leak before detection; it feeds both emitted frames and persisted presentation. Unknown legacy tokens neutralize without lookup. The selected A/B policy changes only the exact-canonical-token presentation rule, not the raw-byte, old-ref refusal, or no-legacy-live-output rules.

The normalizer has a recursive companion, `projectMachineValue`, for decoded tool/native/notification/callback/attachment objects and machine-content fields in REST/MCP/WebSocket payloads. Explicit authenticated Todo identity fields remain governed by the canonical parser/API contract. The walker visits every array element, own string key, and string value; it does not trust role or content field names. It preserves raw evidence, applies a bounded depth/node/byte budget, and fails closed on unsupported values, invalid shapes, or truncation. If two projected keys collide after neutralization, the entire value is rejected—never overwritten, dropped, or merged. The complete message projector dispatches raw `media`, `tool_call`, and `tool_id` through their field-specific validators before walking nested media/tool values; the generic walker never converts a raw protocol ID into prose. It also covers native `tool_use.input`, tool-result content, live `delta.input`, nested notification/callback metadata, and escaped keys/values; plain streamed text uses the boundary-buffering normalizer.
...
| Todo cards and technical refs | Card uses private refs; `publicWorkItemReference` suppresses old scheme | Common floor suppresses canonical IDs from unrelated source/approval labels; policy A adds only the allowlisted visible identity node, policy B renders title/state only | Conditional privacy boundary |
...
The contract phrase “routes/UI accept and emit JIN-N” is conditional only at the selected rendering branch. Authenticated REST transport routes and explicit API/MCP identity fields accept/emit JIN under both policies; “forbidden in URLs” means browser-visible/navigation URL and history, not authenticated REST paths. Policy A gives the identifier human-readable UI utility in allowlisted text/copy/search and uniformly visible exact machine-prose tokens; policy B suppresses it from machine-rendered browser output. Under both, canonical Todo search is special-cased out of history-shareable `location.search`, a user-authored literal remains authored text, and technical anchors remain private.
...
5. Validate every `work_items.id` with shared `parseLegacyTodoId`; any nonmatching, current, arbitrary `wi_*`, or mixed ID aborts. Inventory every class in the graph. Direct references must resolve. Full-scan all nonlegacy `sessions.transport_meta`; raw `messages.content`, `media`, `tool_call`, `tool_id`, `blocks`, `meta`, and per-field origins; callback/queue structures; message PKs/refs; FTS; engine refs; native bindings; and sync state rather than following only relational links. Recognized duplicate shapes are supported; unknown reference-bearing shapes and identity-token-bearing ambiguous field provenance abort.
6. Run hostile-filesystem-safe `scanWorkflowArtifact` on every allowlisted definition/YAML, trigger store, run, invocation claim, active index, legacy state, and wave receipt. Scan raw bytes plus every recursively decoded string key/value with the shared exact finder. Any exact token, mapped or unknown, in nonterminal/executable state aborts. Invalid/corrupt/unreadable/over-limit artifacts, unclassified decoded matches, unsafe names/types/links, identity drift, or unsupported no-follow semantics fail closed; the active index never decides status.
...
11. Require no active/waiting/partial turn, pending old-epoch queue replay, or dirty transcript sync. Inventory every native engine ref/path/binding, verify stable file identity/digest, and prove epoch-1 backfill, tail-sync, late recovery, switch, retry, and resume can be severed before validation.
...
12. Bump the Todo identity epoch and separate message-ref epoch; classify every message field, create the complete map-free presentation, derive opaque tool correlation, and generate one fresh message ref for every row, including duplicates/historical synthetic rows. Replace FTS with the epoch projection and remove raw-content triggers/watermarks. Drop the temporary Todo map, make the manifest unavailable, rebuild presentations again, and require byte-identical output.
13. Sever epoch-1 engine-session/native-binding authority, stamp current engine/queue/sync state, and require future adoption to bind current epoch, authenticated attempt, durable file identity, expected size/offset, and prefix digest.
...
- every message has proven/conservative per-field provenance for content, media labels, tool call, and tool ID; one complete current Todo-epoch presentation; one current message-ref-epoch opaque ref; and valid session-scoped opaque tool correlation. FTS is rebuilt from safe content presentation; nested keys/values pass bounded collision-safe projection; list/page/search/context/read-session/MCP/WebSocket/callback/attachment serializers expose only complete safe messages/public refs; raw/prior/cross-session cursors and raw tool IDs refuse without fallback;
- after the temporary map is dropped and the audit manifest is unavailable, presentation deletion/rebuild is byte-identical for the same sanitizer version and deterministic after a version bump; no historical machine literal is mapped to JIN under either A or B;
- no epoch-1 native engine ref can serialize, resume, backfill, tail-sync, switch, retry, or re-prompt; every current binding verifies authenticated generation, canonical path, durable descriptor identity, expected size/offset, and prefix digest, and native ingress is normalized before emit/store;
...
- Modify `packages/jinn/src/workflows/derive.ts` and `index.ts` to classify legacy `state.json`, wave reports, and YAML definitions and to route every preflight read through the safe scanner
...
RED: raw and decoded parity with `wi_00000000000\\u0061`, `\\u0077i_00000000000a`, and escaped object keys; matches in nonterminal trigger payload keys/values, condition paths/values, node instructions, nested parameters, step override/edit prompts, receipt outcome/artifact/field keys/values, parked/error/detail/report text; running, parked, stopping, dispatched, active-index omitted/extra cases; invalid JSON/YAML and scan depth/size/aggregate limits; parked resume and startup/interval sweep; terminal completed/failed/cancelled raw hash plus sanitized list/detail/MCP/reporting Activity/recovery/session delivery; mutable definition/custom-trigger/invocation-claim classification; legacy `state.json`, wave reports, and `.workflow.yaml` classification; `source=workflow` negative capability; and legacy Workflow session byte checksums.

RED hostile-filesystem fixtures cover a symlinked file and parent, FIFO, character/block device where the test platform permits, socket, sparse/oversize file, unexpected suffix/name, same-size rewrite, replacement between `lstat` and `open`, replacement between `open` and `fstat`, parent replacement after open, truncation/growth during read, per-file/aggregate budget exhaustion, and a platform without trustworthy no-follow/stable-identity support. Every case refuses without blocking or reading foreign bytes; diagnostics contain only artifact class, allowlisted relative locator, phase/reason, and digest.

GREEN: one shared raw-plus-recursively-decoded scanner owns the closed root/name grammar, opens regular files no-follow/nonblocking, verifies descriptor and parent identity, performs bounded reads from the opened descriptor, and provides locator/digest-only structural diagnostics. Any unsupported filesystem guarantee, malformed artifact, or nonterminal legacy-bearing run blocks regardless of index/engine/queue state; no execution projection exists. After blockers terminalize under the legacy epoch, migration succeeds deterministically. New runs emit JIN, terminal evidence stays byte-identical, and every public/reporting/recovery serializer uses safe presentation.
...
- Add `packages/jinn/src/sessions/message-presentations.ts`, `message-refs.ts`, and `native-transcript-bindings.ts`
- Modify `packages/jinn/src/sessions/registry.ts`, `manager.ts`, `rate-limit-handler.ts`, and `fork.ts`
- Modify `packages/jinn/src/gateway/api.ts`, `external-turns.ts`, `files.ts`, `chat-activity.ts`, `hook-registry.ts`, `streamed-blocks.ts`, and `server.ts`
- Modify `packages/jinn/src/engines/platform-context.ts`, `claude-interactive.ts`, `codex.ts`, `codex-interactive.ts`, `grok.ts`, `grok-interactive.ts`, `hermes-acp.ts`, `pi.ts`, `antigravity.ts`, `antigravity-protocol.ts`, `transcript-tailer.ts`, and `mock.ts`; this is the current complete `resumeSessionId` consumer set plus transcript protocol/tailer
- Modify `packages/jinn/src/mcp/search-tools.ts`, `session-tools.ts`, and `file-tools.ts`
- Modify `packages/jinn/src/shared/types.ts`
- Modify `packages/web/src/lib/api.ts`, `blocks.ts`, `conversations.ts`, `packages/web/src/hooks/use-live-session.ts`, `packages/web/src/components/chat/message-media.tsx`, Chat message/anchor consumers, and their tests
- Add focused registry/gateway/MCP/native-transcript/message-reference/FTS fixtures, including existing media, stream-block, fork, and every resume-capable adapter suite

RED content fixtures: raw proven-human prose with an exact legacy/current literal remains byte-identical and non-resolving; A renders authored prose byte-identically, while B masks current canonical tokens only at browser display. Prove `role='user'` machine rows for Workflow spawn/follow-up, delegation, agent-created session, callback/queue replay, and native backfill are not classified human. An identity-bearing ambiguous historical row blocks; an unknown non-bearing row receives conservative presentation without raw rewrite. Persisted assistant, notification, partial, synthetic fallback, tool input/result, thinking, connector output, and split-across-delta tokens expose no machine legacy identity. Every exact historical machine `wi_*` literal becomes the fixed neutral marker under both A and B and is never mapped to JIN. REST detail/page/older-page/search/context/transcript, MCP `read_session`/search/context, connector replies, WebSocket text/tool-result/notification/block/completion, and FTS snippets use the presentation boundary. Delete the temporary map and make the audit manifest unavailable, delete all presentation rows, rebuild twice, and require byte-identical output; bump the sanitizer version and require a deterministic complete rebuild with no manifest/map access. Old raw FTS does not survive rebuild/restart; stale/corrupt projection or FTS epoch fails closed and no post-listen raw-source backfill runs.

RED Policy-A corpus puts the exact same canonical spellings into proven current-epoch machine transcript prose: a real `JIN-42`, a hallucinated `JIN-42`, nonexistent `JIN-999999`, wrong-instance-looking `JIN-424242`, and coincidental prose. A exposes every syntactically exact token as visible non-linking text without any Todo lookup, receipt correlation, existence check, or intent inference; B masks every token. `JIN-0`, `JIN-042`, `jin-42`, and `JIN-42x` are nonmatches. In both policies all technical metadata, anchors, attributes, cursors, tool correlation, and non-presentation fields remain private/opaque.

RED recursive-value fixtures: escaped exact IDs in native `tool_use.input` object keys/values, nested arrays, tool-result objects, live `delta.input`, notification/callback/attachment metadata, and REST/MCP/WS payloads. Split text tokens are buffered; historical decoded machine keys/values are neutralized map-free; depth/node/byte overflow and unsupported/invalid shapes fail closed; two keys that collide after projection reject the whole value. Raw evidence hashes remain unchanged.

RED durable-field fixtures independently exercise `messages.media`, `messages.tool_call`, and `messages.tool_id` with raw literals, escaped keys/values, persistence/reload, FTS-adjacent text, REST page/detail/context/search, MCP read/search/context/file results, WebSocket deltas/finals/attachments, and every engine-ingress path. Verify aligned per-label media provenance preserves proven authored attachment labels under A and applies B display masking; untrusted technical URLs/capabilities never inherit label provenance. For raw `tool_call`, safe non-Todo names may derive a display-only label, while exact legacy/current Todo-shaped values become neutral `Tool` under both policies; neither projected names nor arguments can be replayed as executable input. Raw tool correlation never leaves the store: matching call/result rows receive the same current session-scoped `tc<epoch>_*`, duplicates receive a different session-scoped value, and missing/corrupt pairs fail closed. Existing duplicate-session behavior that omits `tool_call`/`tool_id` is a tested guard-release gap, not assumed current coverage; the guard release copies every field plus provenance atomically.
...
RED native fixtures: epoch-1 JSONL contains text, nested/escaped tool input/result, and thinking with legacy IDs. Raw transcript endpoint, empty-session detail backfill, on-load tail sync, hook adoption, missed-stop recovery, old scalar/every-engine session ref, native resume command, engine switch both directions, rate-limit retry, pending queue replay, and late recovery all refuse epoch-1 authority and insert/emit nothing. For current epoch, cover rotation before first read, rotation after read error, truncate/regrow past the old offset, same-size replacement, path reuse, rename while descriptor remains open, identity drift with matching prefix, prefix rewrite with stable identity, impossible growth, nonregular replacement, process restart/reopen, and same engine-session ID pointing at a different file. Reopen succeeds only when durable identity, expected size/offset, prefix digest, epoch, and path all match; otherwise the binding freezes with zero insert/emit and a fresh authenticated generation/adoption is required. A duplicated session cannot inherit resume authority. Engine switch/re-prompt uses only safe projected context.

GREEN: frozen raw authored/native bytes and internal PKs remain unchanged; complete per-field provenance and map-free deterministic presentations cover content/media/tool call/tool correlation/blocks/meta; every live reader and engine ingress uses current-epoch API/browser/engine projections; FTS is rebuilt only from safe API content; every public message identity is opaque/current/scoped; old native refs are severed; current native adoption is descriptor-bound and authenticated; machine content is normalized before both first frame and persistence.
...
6. Run raw-plus-decoded escaped Workflow fixtures plus hostile-filesystem substitutions and parked resume/sweep; accepted-and-consumed and all-absent callback tombstones; duplicated manager message/completion contract/synthetic Todo and delegation rows; map-destroyed/sanitizer-bumped presentation rebuild; content/media/tool-call/tool-correlation raw/escaped/reload/REST/MCP/WebSocket/engine projections; Policy-A exact-token corpus and Policy B masking; old/current native resume/backfill/tail-sync/engine-switch plus rotation/truncation/path-reuse bindings; Activity list/story/search/rebuild; refusal-only cross-instance; exact legacy-grammar mismatch; opaque-message cursor expiry/reload; and both conditional browser policy fixture branches.
...
| Workflow encoding boundary | Escaped tokens in raw/decoded trigger, condition, prompt, params, receipts, keys; invalid/over-limit JSON/YAML | Shared scanner reports only a hashed structural locator; every nonterminal match blocks; invalid/limit fails closed |
| Workflow filesystem boundary | Symlinked file/parent, FIFO/device/socket, sparse/oversize, unexpected name, stat/open/read replacement, parent drift, budget exhaustion | Closed allowlist plus no-follow descriptor verification refuses every hostile object/race without blocking or reading foreign bytes; diagnostics expose only locator/digest |
...
| Message content presentation | User, assistant, notification, partial, tool result/input, fallback, duplicate, split stream | User prose is byte-identical/non-resolving; raw evidence unchanged; every historical machine legacy token becomes one fixed marker under A/B; current canonical output follows A/B |
| Per-field provenance | Genuine human plus role-user Workflow/delegation/agent/queue/native rows, media labels, tool fields, and ambiguous historical identity text | Guard stamps every populated field at every producer; structural history proof never relies on role; ambiguous identity-bearing field blocks; raw bytes remain frozen |
| Map-free presentation rebuild | Destroy temporary map, make audit manifest unavailable, delete derived rows; repeat at same sanitizer version then bump it | Pure rebuild is byte-identical at one version and deterministic at the next; no manifest/map read or hidden alias exists |
| Durable message fields | `media`, `tool_call`, `tool_id` raw/escaped/reload across REST/MCP/WS/engine; authored and machine labels; duplicated call/result | Complete per-field provenance/projected JSON prevents raw bypass; authored attachment labels follow A/B; legacy/current Todo-shaped tool protocol names neutralize under both; tool input is non-executable; opaque correlation preserves valid pairs without exposing raw IDs |
| Recursive machine values | Escaped IDs in nested tool/native/delta/notification/callback/attachment/media/tool-call keys/values, collision and limit cases | Complete bounded walk neutralizes historical strings map-free; collision/invalid/limit fails closed; no raw nested bypass |
...
| Native history epoch | Old engine refs/JSONL/hook/tailer/switch/retry/replay and current descriptor-bound file with rotation/truncation/path reuse/reopen | Old authority imports/emits nothing; current reads only verified appended bytes; any identity/size/digest/path drift freezes zero-output until fresh authenticated adoption |
| Browser policy A | Todo/detail/Activity/delegation/search/copy plus real, hallucinated, nonexistent, wrong-instance-looking, and coincidental exact canonical transcript tokens | Every syntactically exact token in proven machine prose is visible non-linking text with zero semantic/existence lookup; absent from navigation/history/storage/attributes/anchors/cursors |
...
7. **Message/native evidence boundary:** Preserve raw authored/machine/native bytes and internal PKs while proving durable provenance for content, media labels, tool calls, and tool correlation without trusting role; rebuild map-free presentations without the audit manifest; recursively project nested machine values; and route every serializer, FTS input, stream, connector, engine resume/switch/re-prompt, backfill, and tail-sync through current safe projections. Any ambiguous identity-bearing origin, raw/nested field fallback, manifest-dependent rebuild, or epoch-1 re-entry keeps the instance offline.
...
10. **Browser state:** The common private-ref floor is locked and migration requires clean tabs/no dirty journals. The exact Policy A/B rendering branch remains an operator choice. Stale state is quarantined for explicit copy/recovery; it is never silently discarded and never stores or reconstructs a canonical ID.
...
13. **Hostile Workflow storage:** Verify the preflight's closed locator grammar, no-follow descriptor checks, bounded reads, parent stability, and locator/digest-only diagnostics on every supported platform. Missing trustworthy primitives are a platform no-go, not a reason to fall back to ordinary path reads.
14. **Native file capability:** Verify adoption binds an authenticated engine generation to durable file identity, size/offset, prefix digest, epoch, and canonical path. Rotation, path reuse, truncation, identity drift, or unverifiable reopen freezes ingestion and requires fresh authenticated adoption.
15. **Policy A mechanics:** If selected, A permits every exact canonical token in proven current-epoch machine transcript prose as non-linking visible text; it never infers intent or existence. If that broad mechanical rule is unacceptable, select B rather than adding lookup-driven semantics.
...
- content/media/tool-call/tool-correlation provenance is durable for new producers, historical role-user machine rows cannot be mistaken for humans, nested decoded values are bounded/collision-safe, and message refs remain stable within a separately guarded ref epoch;
- every historical machine legacy literal neutralizes map-free under both policies, presentation rebuild succeeds without the temporary map/audit manifest, and Policy A/B exact-canonical-token behavior is mechanically testable without lookup;
- Workflow raw and recursively decoded JSON/YAML keys/values share one exact parser and hostile-filesystem scanner and fail closed;
- current native transcript bindings survive restart only when descriptor identity, size/offset, prefix digest, epoch, and path remain valid; any new file requires authenticated adoption;
...
- any Workflow managed artifact was not scanned in both raw and fully decoded string-key/value form where structured, any legacy state/wave/YAML surface is unclassified, invalid/over-limit structured data can pass, or filesystem traversal can follow/block on/unboundedly read a hostile object;
...
- any nonlegacy `transport_meta`, `messages.content`, `messages.media`, `messages.tool_call`, `messages.tool_id`, `messages.meta`, or `messages.blocks` row with an exact legacy Todo reference is unclassified or missed by the full scan;
...
- an identity-bearing historical message has ambiguous authorship, any new producer can omit per-field provenance, media/tool fields bypass complete presentation, raw tool IDs escape, nested machine values bypass the recursive projector, a presentation rebuild needs the temporary map/manifest, or random message refs can be replaced without incrementing the message-ref epoch;
- Policy A depends on semantic intent/existence lookup, does not expose every exact canonical token in proven current-epoch machine prose uniformly, or permits a canonical token in a technical anchor/metadata/cursor;
- any historical machine legacy literal maps to JIN rather than the common neutral marker, or destroy-and-rebuild/sanitizer-version-bump output is nondeterministic;
- Workflow scanning cannot prove no-follow regular-file identity and bounded-read invariants on the deployment filesystem, or diagnostics can disclose raw/foreign bytes;
- native transcript adoption lacks authenticated generation, durable file identity, expected size/offset, prefix digest, epoch, and canonical path, or any replacement/truncation/rotation/path-reuse condition can auto-follow or emit bytes;
...
## V5 Re-Audit Evidence

The v4 rejection and every previously accepted correction were re-audited against the current source before this revision. These are the load-bearing paths:
...
- Durable omitted message fields: `sessions/registry.ts:506-538,2386-2417,2432-2478,2508-2625,2811-2824` defines, parses, stores, reloads, and directly returns `media`, `tool_call`, and `tool_id`. `duplicateSession` at `:2287-2339` currently copies `media` but omits both tool columns. Engine streaming persists and emits tool identity at `gateway/api.ts:7499-7544,7651-7660,7811-7818`; attachment paths at `gateway/api.ts:5681-5688`, `gateway/files.ts:989-1008,1057-1071`, and `mcp/file-tools.ts:196-210` create media-bearing messages. Web reload/live consumers at `hooks/use-live-session.ts:270-287,748-899`, `lib/conversations.ts:29-57`, and `components/chat/message-media.tsx:141-160` use these fields and render attachment labels. All three therefore require durable per-field provenance and complete API/browser/engine projection.
- Policy-A mechanics and map-free rebuild: current stream/message rows carry ordinary content plus tool fields but no authenticated token-span/semantic-intent record (`sessions/registry.ts:2386-2417,2508-2529`; `gateway/api.ts:7651-7660,8085-8094`). A real, hallucinated, nonexistent, wrong-instance-looking, or coincidental exact canonical token is structurally indistinguishable after persistence. V5 consequently makes A grammar-only for proven machine prose and makes both policies neutralize historical machine legacy literals, so `message_presentations` is a pure frozen-row/provenance/epoch/policy/version/key derivation after the Todo map and manifest are unavailable.
...
- Native path replacement: `gateway/external-turns.ts:111-132,161-190,245-260,341-366` reads and rediscovers transcript paths and gates mainly by timestamps; `engines/transcript-tailer.ts:26-72` stats by path and reopens after read errors. Codex/Grok/Antigravity adapters discover/stat candidate files (`codex-interactive.ts:56-78,295-348`; `grok-interactive.ts:58-84,265-315`; `antigravity.ts:169-233`; `antigravity-protocol.ts:27-44,215-240`). Path+offset is therefore not authority: a current binding must persist authenticated generation, canonical path, stable file identity, expected size/offset, and prefix digest, and freeze on any drift.
- Workflow filesystem boundary: managed roots and legacy surfaces are read with ordinary path APIs in `workflows/run-store.ts:506-560,820-842,948-1001`, `definition-store.ts:150-164,222-264,380-397`, `custom-triggers.ts:522-538`, `derive.ts:10-17,164-207,259-266`, and `index.ts:48-58,108-118`. Those paths cover definitions/YAML, triggers, runs, active index, invocation claims, legacy `state.json`, and wave reports but currently follow links and can race replacement. V5 therefore gives preflight a closed locator grammar, no-follow/nonblocking descriptor verification, parent stability checks, hard per-file/aggregate bounds, and locator/digest-only diagnostics.
...
- Session schema and durable state: `packages/jinn/src/sessions/registry.ts`, `callbacks.ts`, `delegation-completion-contract.ts`, `manager.ts`, `rate-limit-handler.ts`, `fork.ts`, planned `message-presentations.ts`, planned `message-refs.ts`, and planned `native-transcript-bindings.ts`.
- Gateway producers/consumers: `packages/jinn/src/gateway/api.ts`, `external-turns.ts`, `files.ts`, `server.ts`, `chat-activity.ts`, `manager-visibility.ts`, `hook-registry.ts`, and `streamed-blocks.ts`.
- Workflow evidence and triggers: `packages/jinn/src/workflows/run-store.ts`, `definition-store.ts`, `run-reconciler.ts`, `reporting.ts`, `handoff.ts`, `todo-status-trigger.ts`, `custom-triggers.ts`, `condition.ts`, `advance.ts`, `derive.ts`, and `index.ts`.
...
- Engine/native history and resume: `packages/jinn/src/engines/platform-context.ts`, `claude-interactive.ts`, `codex.ts`, `codex-interactive.ts`, `grok.ts`, `grok-interactive.ts`, `hermes-acp.ts`, `pi.ts`, `antigravity.ts`, `antigravity-protocol.ts`, `transcript-tailer.ts`, and the test `mock.ts`; repeat the literal consumer sweep at implementation time to catch repository drift.
...
- Web privacy/data/CAS: `packages/web/src/lib/todos.ts`, `query-client.ts`, `conversations.ts`, planned `chat-private-anchors.ts`, `packages/web/src/hooks/use-live-session.ts`, `packages/web/src/routes/todos/todo-private-state.ts`, `todo-edit-request.ts`, `use-todo-quick-edit.ts`, `use-todo-draft.ts`, `use-todos.ts`, Todo `page.tsx`, `group.tsx`, `row.tsx`, `detail-sheet.tsx`, Chat `routes/chat/page.tsx`, and `packages/web/src/components/chat/chat-messages.tsx`, `message-media.tsx`, `handoff-card.tsx`, `dispatch-row.tsx`, `chat-blocks.tsx`, `comms-callout.tsx`, `thread-peek.tsx`, plus `company-activity-card.tsx`.
...
- Sessions: `packages/jinn/src/sessions/registry.test.ts`, `__tests__/registry-delete-queue-items.test.ts`, `callback-deliveries.test.ts`, `callback-concurrent-init.test.ts`, `delegation-completion-contract.test.ts`, `legacy-workflow-session-compat.test.ts`, `messages-media.test.ts`, `messages-partial.test.ts`, duplication/fork tests, and message-search/context tests.
...
- Web: Todo route detail/page/history/private-state/edit/draft/quick-edit/CAS/pagination tests; `packages/web/src/lib/__tests__/todos.test.ts`, `company-activity-blocks.test.ts`, planned private-anchor tests; `message-media.test.tsx`, `chat-blocks.test.tsx`, `chat-messages-jump.test.tsx`, `chat-messages-tool-group.test.tsx`, `comms-v2.test.tsx`, activity-card/parity tests, route-level preview/history/focus tests, and live-session/query-invalidation tests.
...
- [x] `messages.media`, `messages.tool_call`, and `messages.tool_id` are explicit durable channels with per-field provenance, complete current-epoch projection, authored attachment-label policy, two-policy-neutral technical tool names, opaque tool correlation, and raw/escaped/reload/REST/MCP/WebSocket/engine coverage.
- [x] Durable per-field origin distinguishes proven human prose/labels from role-user machine prompts and engine protocol; ambiguous identity-bearing history blocks, and nested decoded machine values are recursively bounded with collision failure.
- [x] Historical machine legacy literals neutralize map-free under both policies; presentation rebuild is deterministic after map/manifest destruction and sanitizer-version change.
- [x] Policy A is grammar-only and lookup-free for every exact current canonical token in proven machine transcript prose; Policy B masks those tokens; technical identity remains private/opaque in both.
...
- [x] Workflow preflight covers the closed JSON/YAML/legacy state/wave artifact set with symlink/nonregular/race-safe descriptor reads, hard budgets, and locator/digest-only diagnostics.
- [x] Native transcript adoption binds authenticated engine generation, canonical path, stable file identity, expected size/offset, prefix digest, and epoch; rotation/truncation/path reuse freezes rather than auto-follows.
...
- [x] Browser technical privacy is locked; the mechanically specified visible-token A/B choice is explicitly conditional with downstream tests.
